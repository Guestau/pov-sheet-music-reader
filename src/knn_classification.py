...
class Classification:
    def __init__(self):
        ...
        if os.path.exists('knn_data.npz'):
            ...
            x = np.array(cells)
            train = x[:,:3*self.xcount/4].reshape(-1,self.xbox*self.ybox).astype(np.float32) # Size = (xcout*ycount/2,xbox*ybox)
            test = x[:,3*self.xcount/4:self.xcount].reshape(-1,self.xbox*self.ybox).astype(np.float32) # Size = (xcout*ycount/2,xbox*ybox)
