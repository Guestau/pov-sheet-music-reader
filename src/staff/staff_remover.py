...
from note_head_detector import WholeNoteHeadDetector, HalfNoteHeadDetector, AllNoteHeadsDetector
    ...
    def remove_at(self, peak_index, new_image=None, peak_tolerance=True):
        ...
        for pixel_x in xrange(new_image.shape[1]):
            ...
            for i in xrange(-peak_tolerance * staff_line_height, peak_tolerance * staff_line_height + 1):
                if peak_index + i > 0 and peak_index + i < new_image.shape[0] and new_image[
                            peak_index + i, pixel_x] == 0:
                    peak = peak_index + i
                    break
